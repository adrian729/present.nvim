...
local function create_floating_window(config, enter)
  ...
  -- Create a buffer
  local buf = vim.api.nvim_create_buf(false, true) -- No line, scratch buffer
...
local create_window_configuration = function()
  ...
  local footer_height = 1 + 2 -- no border
  local body_height = height - header_height - footer_height - 2 -
      1                       -- for our own border and body top padding (start at 4)
...
M.start_presentation = function(opts)
  ...
  vim.api.nvim_create_autocmd("BufLeave", {
    ...
    callback = function()
      -- Reset the values when we are done presenting
      for option, config in pairs(restore) do
  ...
  vim.api.nvim_create_autocmd("VimResized", {
    ...
    callback = function()
      ...
      foreach_float(function(name, _)
        vim.api.nvim_win_set_config(state.floats[name].win, updated[name].body)
      end)
